...
public class BatchEvaluatorPercentage implements Evaluator<EnumGene<Integer>, Integer> {
     ...
     */
    public BatchEvaluatorPercentage(SlotAllocationProblem problem, JeneticsOptimization optimization) {
...
//            }
        ...
        if(this.optimization.getFitnessEstimator() != null) {
            logger.debug("Assign the top individuals the maximum fitness, the others receive INTEGER.MIN_VALUE");
            estimatedPopulationStream = evaluatedPopulation.stream()
                    .map(phenotype -> phenotype.withFitness(evaluatedPopulation.indexOf(phenotype) < 0.3 * population.size() ? (int) maxFitness : Integer.MIN_VALUE));
