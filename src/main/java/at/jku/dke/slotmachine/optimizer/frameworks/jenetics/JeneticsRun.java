...
public class JeneticsRun extends Run {
	...
	public static Map<Flight, Slot> run(List<Flight> flights, List<Slot> slots, JeneticsConfig jenConfig, Optimization opt) {
		...
		if (true) {
			return runFlightListPermutationProblem(flights, slots, jenConfig, opt);
		}
...
//        		);
	...
	// EXPERIMENTAL METHODS for FlightListPermutationProblem
	private static Map<Flight, Slot> runFlightListPermutationProblem(List<Flight> flights, List<Slot> slots,
			JeneticsConfig jenConfig, Optimization opt) {

		logger.info("Running altered version (FlightListPermutationProblem) of Jenetics ...");
		FlightListPermutationProblem p = new FlightListPermutationProblem(
				ISeq.of(flights), ISeq.of(slots));
		//Constraint<EnumGene<Integer>, Integer> constraint = p.constraint().get();
		
		// engine takes mutator and crossover from jenConfig - if no alterer detected default values are used
		Engine<EnumGene<Flight>, Integer> e = Engine.builder(p)
				.populationSize(getPopulationSize(jenConfig))
				.alterers(
						getMutatorFLPP(jenConfig),
						getCrossoverFLPP(jenConfig))
				.offspringSelector(getOffspringSelectorFLPP(jenConfig))
				.survivorsSelector(getSurvivorSelectorFLPP(jenConfig))
				.maximalPhenotypeAge(getMaximalPhenotypeAge(jenConfig))
				.offspringFraction(getOffspringFraction(jenConfig))
				//.constraint(constraint)
				.build();
        ...
        Genotype<EnumGene<Flight>> result = null;
        int randomNr = 963;
        List<Predicate<? super EvolutionResult<EnumGene<Flight>, Integer>>> limits = getLimitsFLPP(jenConfig);
        if (limits != null) {
	        switch (limits.size()) {
	        	case 1:
	        		logger.info("1 termination method detected: " + limits.get(0).toString());
        ...
        ISeq<Flight> resultISeq = p.decode(result);
        
        Map<Flight, Slot> resultMap = new HashMap<Flight, Slot>();
        int i = 0;
        for (Flight f: resultISeq) {
        	resultMap.put(f, slots.get(i));
        	i++;
        }
	...
	// UTILITY METHODS (for FlightListPermutationProblem)
	/**
	 * Gets the crossover method according to jenConfig. If no crossover method is detected
	 * it returns the default value (PartiallyMatchedCrossover<>(0.35))
	 * 
	 ...
	 * @return crossover method (as an object, e.g. PartiallyMatchedCrossover<>(0.35))
	 */
	private static Crossover<EnumGene<Flight>, Integer> getCrossoverFLPP(JeneticsConfig jenConfig) {
		if (jenConfig.getAlterer() != null && jenConfig.getAlterer().length > 0) {
			Alterer alterer0 = jenConfig.getAlterer()[0];
			Alterer alterer1 = null;
			if (jenConfig.getAlterer().length == 2) {
	 ...
	 * @return crossover method (as an object, e.g. SwapMutator<EnumGene<Integer>, Integer>(0.2))
	 */
	private static Mutator<EnumGene<Flight>, Integer> getMutatorFLPP(JeneticsConfig jenConfig) {
		if (jenConfig.getAlterer() != null && jenConfig.getAlterer().length > 0) {
			Alterer alterer0 = jenConfig.getAlterer()[0];
			Alterer alterer1 = null;
			if (jenConfig.getAlterer().length == 2) {
			...
			if (crossover0) {
				...
				if (alterer0.equals(Alterer.SWAPMUTATOR)) {
					if (jenConfig.getAltererAttributes() != null && jenConfig.getAltererAttributes().length > 0 &&
						...
						double parameter = jenConfig.getAltererAttributes()[0][0];
						logger.debug("Mutator: SwapMutator will be used with probability of " + parameter + ".");
						return new SwapMutator<EnumGene<Flight>, Integer>(parameter);
					}
				}
				// possible other crossover (with (0 or 1) or (0 or 1 or 2) parameters required)
		...
		return new SwapMutator<EnumGene<Flight>, Integer>(0.2);
	}
	 ...
	 * @return selector (as an object, e.g. TournamentSelector<EnumGene<Integer>, Integer>())
	 */
	private static Selector<EnumGene<Flight>, Integer> getSurvivorSelectorFLPP(JeneticsConfig jenConfig) {
		if (jenConfig == null || jenConfig.getSurvivorSelector() == null) {
			logger.debug("TournamentSelector will be used as the survivor selector.");
			return new TournamentSelector<EnumGene<Flight>, Integer>();
		}
		logger.debug("The survivor selector, which will be used is:");
		return getSelectorFLPP(jenConfig.getSurvivorSelector(), jenConfig.getSurvivorSelectorAttributes());
	}
	 ...
	 * @return selector (as an object, e.g. TournamentSelector<EnumGene<Integer>, Integer>())
	 */
	private static Selector<EnumGene<Flight>, Integer> getOffspringSelectorFLPP(JeneticsConfig jenConfig) {
		if (jenConfig == null || jenConfig.getOffspringSelector() == null) {
			logger.debug("TournamentSelector will be used as the offspring selector.");
			return new TournamentSelector<EnumGene<Flight>, Integer>();
		}
		logger.debug("The survivor selector, which will be used is:");
		return getSelectorFLPP(jenConfig.getOffspringSelector(), jenConfig.getOffspringSelectorAttributes());
	}
	 ...
	 * @return selector (as an object, e.g. TournamentSelector<EnumGene<Integer>, Integer>())
	 */
	private static Selector<EnumGene<Flight>, Integer> getSelectorFLPP(
			at.jku.dke.slotmachine.optimizer.service.dto.JeneticConfigDTO.Selector selector,
			double[] selectorAttributes) {
		// if no selector can be detected, should usually not happen
		if (selector == null) {
		...
		switch (selector) {
			case BOLTZMANNSELECTOR: 
				if (selectorAttributes == null || selectorAttributes.length == 0) {
					logger.debug("BoltzmannSelector");
					return new BoltzmannSelector<EnumGene<Flight>, Integer>();
				}
				logger.debug("BoltzmannSelector (b = " + parameter0 + ")");
				return new BoltzmannSelector<EnumGene<Flight>, Integer>(parameter0);
			case EXPONENTIALRANKSELECTOR:
				if (selectorAttributes == null || selectorAttributes.length == 0) {
					logger.debug("ExponentialRankSelector");
					return new ExponentialRankSelector<EnumGene<Flight>, Integer>();
				}
				logger.debug("ExponentialRankSelector (c  = " + parameter0 + ")");
				return new ExponentialRankSelector<EnumGene<Flight>, Integer>(parameter0);
			case LINEARRANKSELECTOR:
				if (selectorAttributes == null || selectorAttributes.length == 0) {
					logger.debug("LinearRankSelector");
					return new LinearRankSelector<EnumGene<Flight>, Integer>();
				}
				logger.debug("LinearRankSelector (nminus = " + parameter0 + ")");
				return new LinearRankSelector<EnumGene<Flight>, Integer>(parameter0);
			case ROULETTEWHEELSELECTOR:
				logger.debug("RouletteWheelSelector");
				return new RouletteWheelSelector<EnumGene<Flight>, Integer>();
			case STOCHASTICUNIVERSALSELECTOR:
				logger.debug("StochasticUniversalSelector");
				return new StochasticUniversalSelector<EnumGene<Flight>, Integer>();
			case TOURNAMENTSELECTOR:
				if (selectorAttributes == null || selectorAttributes.length == 0) {
					logger.debug("TournamentSelector");
					return new TournamentSelector<EnumGene<Flight>, Integer>();
				...
				logger.debug("TournamentSelector (sampleSize = " + parameter0 + ")");
				return new TournamentSelector<EnumGene<Flight>, Integer>((int) parameter0);
			case TRUNCATIONSELECTOR:
				if (selectorAttributes == null || selectorAttributes.length == 0) {
					logger.debug("TruncationSelector");
					return new TruncationSelector<EnumGene<Flight>, Integer>();
				}
				logger.debug("TruncationSelector (n = " + parameter0 + ")");
				return new TruncationSelector<EnumGene<Flight>, Integer>((int) parameter0);
			default:
				logger.debug("TournamentSelector (no selector chosen)");
				return new TournamentSelector<EnumGene<Flight>, Integer>(); 
		}
	 ...
	 * @return
	 */
	private static List<Predicate<? super EvolutionResult<EnumGene<Flight>, Integer>>> getLimitsFLPP(JeneticsConfig jenConfig) {
		if (jenConfig == null || jenConfig.getTermination() == null) {
			logger.debug("No termination methods detected.");
			return null;
		}
		List<Predicate<? super EvolutionResult<EnumGene<Flight>, Integer>>> terminationList = new LinkedList<Predicate<? super EvolutionResult<EnumGene<Flight>, Integer>>>();
		switch (jenConfig.getTermination().length) {
			case 0:
				logger.debug("No chosen termination methods detected.");
				return null;
			case 1:
				if (jenConfig.getTerminationAttributes() == null || jenConfig.getTerminationAttributes().length < 1) {
					logger.debug("No attributes detected for the termination method.");
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], new double[0]));
				} else {
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], jenConfig.getTerminationAttributes()[0]));
				}
				logger.debug("Termination method " + terminationList.get(0).toString() + " will be used.");
				return terminationList;
			case 2:
				if (jenConfig.getTerminationAttributes() == null || jenConfig.getTerminationAttributes().length < 1) {
					logger.debug("No attributes detected for the termination method.");
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], new double[0]));
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[1], new double[0]));
				} else {
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], jenConfig.getTerminationAttributes()[0]));
					...
					if (jenConfig.getTerminationAttributes().length < 2) {
						...
						terminationList.add(getLimitFLPP(jenConfig.getTermination()[1], jenConfig.getTerminationAttributes()[1]));
					}
				}
				logger.debug("Termination methods " + terminationList.get(0).toString() + " and " 
						+ terminationList.get(1) + " will be used.");
			...
			case 3:
				if (jenConfig.getTerminationAttributes() == null || jenConfig.getTerminationAttributes().length < 1) {
					...
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], new double[0]));
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[1], new double[0]));
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[2], new double[0]));
				} else {
					terminationList.add(getLimitFLPP(jenConfig.getTermination()[0], jenConfig.getTerminationAttributes()[0]));
					...
					if (jenConfig.getTerminationAttributes().length < 2) {
						...
						if (jenConfig.getTerminationAttributes().length < 3) {
							...
							terminationList.add(getLimitFLPP(jenConfig.getTermination()[2], jenConfig.getTerminationAttributes()[2]));
						}
					}
				}
				logger.debug("Termination methods " + terminationList.get(0).toString() + " and " 
	 ...
	 * @return termination method
	 */
	private static Predicate<? super EvolutionResult<EnumGene<Flight>, Integer>> getLimitFLPP(Termination termination,
			double[] attributes) {
